...
const addVANToQueue = async (userId: string): Promise<void> => {
  ...
  if (!user) {
    throw new AppError(`User ${userId} does not exist`);
  }
...
const walletWorker = new Worker<VANPayload>(
  "walletQueues",
  async (job: Job<VANPayload>) => {
    ...
    try {
      ...
      log.info(
        `Flutterwave API Response:  ${JSON.stringify(virtualAccountResponse.data, null, 2)}`
      );
      ...
      if (
        ...
        !["Transaction in progress", "success"].includes(response_message)
      ) {
        throw new AppError(
      ...
      const user = await userRepo.findOneBy({ id: userId });
      if (!user) {
        throw new AppError(`User not found for ID: ${userId}`, "404", false);
      }
      ...
      if (wallet.virtualAccountNumber === null) {
        ...
        log.info(
          `Virtual account successfully created for User ID . with account number: ${wallet.virtualAccountNumber}`
        );
      }
